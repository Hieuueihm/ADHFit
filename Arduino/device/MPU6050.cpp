...
double mpu6050Measure(){
  ...
  // Serial.print("MSQ Acceleration: ");
  // Serial.println(a_msq);
  for (int i=15; i>0; i--){
    y[i] = y[i-1];
    // x[i] = x[i-1];
  }
...
bool stepCountAlgorithm(){
  ...
  if (peakDetection(state_range, state_window, state_threshold)){
      return true;
  }
  return false;
}
